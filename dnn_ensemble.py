...
class MyEnsemble(nn.Module):
    def __init__(self, model_list):
        ...
        #for i in range(len(self.model_list)):
            #self.model_list[i].qa_outputs = nn.Identity()
            #print (self.model_list[i])

        # Create new classifier
        self.fc1 = nn.Linear(2*len(model_list), 512)
    ...
    def forward(self,
        ...
        '''
        out = torch.cat(tuple(output_list), dim=1)
        out = self.fc1(out)
        out = self.fc2(out)
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    cached_features_file = os.path.join(
        ...
        "cached_{}_{}_{}".format(
            ...
            list(filter(None, args.cache_dir.split("/"))).pop(),
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for i, feature_index in enumerate(feature_indices):
            ...
            if len(output) >= 5:
                ...
                start_logits, end_logits, hidden_states = output
...
def get_ensemble_result(args):
    ...
    model_path_list = args.input_models.split(',')
    model_list = []
...
def main():
    ...
    parser.add_argument('--input_models', type=str, default="pytorch.bin")
    # Required parameters
    parser.add_argument(
        "--model_type",
        default=None,
